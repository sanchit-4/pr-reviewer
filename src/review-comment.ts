...
import * as github from '@actions/github'
import {Octokit} from '@octokit/action'
import {Bot, ConversationHistory} from './bot.js' // Import ConversationHistory
import {
  Commenter,
  COMMENT_REPLY_TAG,
  COMMENT_TAG,
...
export const handleReviewComment = async (
  ...
  if (
    ...
    const [, begin_history] = await bot.chat(
      prompts.render_comment_beginning(inputs),
    ...
    core.info(
      `Skipped: comment does not mention '${ASK_BOT}' and is not part of a bot conversation.`
    )
  }
}