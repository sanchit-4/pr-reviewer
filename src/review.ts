...
import {Octokit} from '@octokit/action'
import pLimit from 'p-limit'
import {Bot, ConversationHistory} from './bot.js' // Import the history type
import {Commenter, COMMENT_REPLY_TAG, SUMMARIZE_TAG} from './commenter.js'
import {Inputs, Options, Prompts} from './options.js'
import * as tokenizer from './tokenizer.js'
...
export const codeReview = async (
  ...
  const files_to_review = (
    await Promise.all(
      filtered_files.map(async file => {
        ...
        const patches: [number, string][] = split_patch(file.patch).map(
          patch => [patch_comment_line(patch), patch]
        )
        if (patches.length > 0) {
          return [file.filename, file_content, file.patch || '', patches] as [
            string,
            string,
            string,
            [number, string][]
  ...
  if (files_to_review.length > 0) {
    // Summary Stage
    const [summarize_begin_response, summarize_begin_history] = await bot.chat(
      prompts.render_summarize_beginning(inputs),
    ...
    const generateSummary = async (
      ...
      if (file_diff_tokens < options.max_tokens_for_extra_content) {
        try {
          const [summarize_resp] = await bot.chat(
            prompts.render_summarize_file_diff(ins),
            summarize_begin_history // Use the history from the beginning of the summary
          )
    ...
    const summaryPromises = files_to_review.map(
      ([filename, file_content, file_diff]) =>
        concurrency_limit(() =>
          generateSummary(filename, file_content, file_diff)
        )
    )
    const summaries = (await Promise.all(summaryPromises)).filter(
      (s): s is [string, string] => s !== null
    )
    ...
    // Final Summary
    const [summarize_final_response, summarize_final_history] =
      await bot.chat(prompts.render_summarize(inputs), summarize_begin_history)
    ...
    // Final Release Notes
    const [release_notes_response] = await bot.chat(
      prompts.render_summarize_release_notes(inputs),
      summarize_final_history // Continue the conversation from the final summary
    )

    if (release_notes_response) {
      let message = '### Summary by Gemini\n\n' + release_notes_response
      await commenter.update_description(
        context.payload.pull_request.number,
        message
      )
    }
    ...
    // Review Stage
    const [, review_begin_history] = await bot.chat(
      prompts.render_review_beginning(inputs),
      [] // Start a new conversation for the review stage
    )
    ...
    const reviewPromises = files_to_review.map(
      ([filename, file_content, file_diff, patches]) =>
        concurrency_limit(async () => {
          ...
          const [, patch_begin_history] = await bot.chat(
            prompts.render_review_patch_begin(ins),
            file_review_history
          )
...
const split_patch = (patch: string | null | undefined): string[] => {
  ...
  return patch.split(pattern).slice(1)
    .reduce((acc: string[], val: string, i: number) => {
        if (i % 2 === 0) acc.push(val + (patch.split(pattern).slice(1)[i+1] || ''));
        return acc;
    }, []);
}
