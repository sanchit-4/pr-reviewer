...
// import { Part, SingleRequestOptions, GenerateContentResult } from '@google/generative-ai'
...
//   gemini_retries(arg0: (request: string | Array<string | Part>, requestOptions?: SingleRequestOptions) => Promise<GenerateContentResult>, arg1: string[], gemini_retries: any) {
...
//     gemini_model: string
//     gemini_model_temperature: number | undefined
//   constructor(
//     debug: boolean,
//     max_files = '60',
...
export class Options {
    ...
    path_filters;
    system_message;
    gemini_model; // Renamed
    gemini_model_temperature; // Renamed
    gemini_retries; // Renamed
    gemini_concurrency_limit; // Renamed
    max_tokens_for_extra_content;
    constructor(debug, max_files = '60', review_comment_lgtm = false, path_filters = null, system_message = '', gemini_model = 'gemini-1.5-flash', // Default to a modern, fast model
    gemini_model_temperature = '0.7', gemini_retries = '3', gemini_concurrency_limit = '4') {
        this.debug = debug;
        this.max_files = parseInt(max_files);
        this.review_comment_lgtm = review_comment_lgtm;
        this.path_filters = new PathFilter(path_filters);
        ...
        this.gemini_model_temperature = parseFloat(gemini_model_temperature);
        this.gemini_retries = parseInt(gemini_retries);
        this.gemini_concurrency_limit = parseInt(gemini_concurrency_limit);
        // Set a high token limit suitable for Gemini models
        this.max_tokens_for_extra_content = 32000;
    }
    check_path(path) {
        const ok = this.path_filters.check(path);
        core.info(`checking path: ${path} => ${ok}`);
