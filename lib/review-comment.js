...
import * as github from '@actions/github';
import { Octokit } from '@octokit/action';
import { Commenter, COMMENT_REPLY_TAG, COMMENT_TAG, SUMMARIZE_TAG } from './commenter.js';
...
export const handleReviewComment = async (bot, options, prompts) => {
    const commenter = new Commenter();
    const inputs = new Inputs();
    if (context.eventName !== 'pull_request_review_comment') {
        core.warning(`Skipped: ${context.eventName} is not a pull_request_review_comment event`);
        return;
    }
    if (!context.payload || !context.payload.comment) {
        core.warning(`Skipped: event payload is missing comment`);
        return;
    }
    const comment = context.payload.comment;
    if (!context.payload.pull_request || !context.payload.repository) {
        core.warning(`Skipped: event is missing pull_request or repository payload`);
        return;
    }
    inputs.title = context.payload.pull_request.title;
    if (context.payload.pull_request.body) {
    ...
    if (context.payload.action !== 'created') {
        core.warning(`Skipped: comment event is not 'created'`);
        return;
    }
    // Check if the comment is from the bot itself to avoid loops
    if (comment.body.includes(COMMENT_TAG) ||
        comment.body.includes(COMMENT_REPLY_TAG)) {
        core.info(`Skipped: comment is from the bot itself`);
        return;
    }
    const pull_number = context.payload.pull_request.number;
    ...
    if (comment_chain.includes(COMMENT_TAG) ||
        comment_chain.includes(COMMENT_REPLY_TAG) ||
        comment.body.trim().startsWith(ASK_BOT)) {
        inputs.comment = `${comment.user.login}: ${comment.body}`;
        inputs.diff = comment.diff_hunk;
        inputs.filename = comment.path;
        inputs.comment_chain = comment_chain;
        ...
        const summary = await commenter.find_comment_with_tag(SUMMARIZE_TAG, pull_number);
        if (summary) {
            inputs.summary = summary.body;
        }
        // --- Start Conversation with Gemini ---
        // Start with a new, empty history for this reply
        let conversation_history = [];
        const [, begin_history] = await bot.chat(prompts.render_comment_beginning(inputs), conversation_history);
        conversation_history = begin_history;
        if (file_content) {
            inputs.file_content = file_content;
            // NOTE: tokenizer is now async
            const file_content_tokens = await tokenizer.get_token_count(file_content);
            if (file_content_tokens < options.max_tokens_for_extra_content) {
                const [, file_content_history] = await bot.chat(prompts.render_comment_file(inputs), conversation_history);
                conversation_history = file_content_history;
            }
        }
        if (file_diff) {
            inputs.file_diff = file_diff;
            ...
            if (file_diff_tokens < options.max_tokens_for_extra_content) {
                const [, file_diff_history] = await bot.chat(prompts.render_comment_file_diff(inputs), conversation_history);
                conversation_history = file_diff_history;
            }
        }
        const [reply] = await bot.chat(prompts.render_comment(inputs), conversation_history);
        if (reply) {
            if (topLevelComment) {
            ...
            core.info("No reply was generated by the bot.");
        }
    }
    else {
        core.info(`Skipped: comment does not mention '${ASK_BOT}' and is not part of a bot conversation.`);
    }
};
