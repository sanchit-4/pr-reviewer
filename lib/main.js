...
import { codeReview } from './review.js';
async function run() {
    const options = new Options(core.getBooleanInput('debug'), core.getInput('max_files'), core.getBooleanInput('review_comment_lgtm'), core.getMultilineInput('path_filters'), core.getInput('system_message'), core.getInput('gemini_model'), // Changed
    ...
    try {
        ...
        core.warning(`Skipped: failed to create bot, please check your gemini_api_key: ${e}, backtrace: ${e.stack}`);
        return;
    }
    try {
        // check if the event is pull_request
