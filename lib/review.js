...
export const codeReview = async (bot, options, prompts) => {
    ...
    // Use the gemini concurrency limit from options
    const concurrency_limit = pLimit(options.gemini_concurrency_limit);
    ...
    const filtered_files = files.filter(file => options.check_path(file.filename));
    if (filtered_files.length > options.max_files && options.max_files > 0) {
        core.warning("Skipped: too many files to review, can't handle it");
        await commenter.comment(`Skipped: too many files to review, can't handle it`, SUMMARIZE_TAG, 'replace');
        return;
    }
    const files_to_review = (await Promise.all(filtered_files.map(async (file) => {
        let file_content = '';
        try {
            const contents = await octokit.repos.getContent({
                owner: repo.owner,
            ...
            if (contents.data &&
                ...
                'content' in contents.data) {
                file_content = Buffer.from(contents.data.content, 'base64').toString();
            }
        }
        catch (error) {
            core.warning(`Failed to get file contents for ${file.filename}: ${error}, skipping.`);
        }
        const patches = split_patch(file.patch).map(patch => [patch_comment_line(patch), patch]);
        if (patches.length > 0) {
            return [file.filename, file_content, file.patch || '', patches];
        }
        return null;
    }))).filter(file => file !== null);
    if (files_to_review.length > 0) {
        // Summary Stage
        const [summarize_begin_response, summarize_begin_history] = await bot.chat(prompts.render_summarize_beginning(inputs), [] // Start with an empty history
        );
        const generateSummary = async (filename, file_content, file_diff) => {
            ...
            if (file_diff_tokens < options.max_tokens_for_extra_content) {
                try {
                    const [summarize_resp] = await bot.chat(prompts.render_summarize_file_diff(ins), summarize_begin_history // Use the history from the beginning of the summary
                    );
                    return summarize_resp ? [filename, summarize_resp] : null;
                }
                catch (error) {
                    core.warning(`summarize: error from gemini: ${error}`);
                    return null;
                }
            }
            return null;
        };
        const summaryPromises = files_to_review.map(([filename, file_content, file_diff]) => concurrency_limit(() => generateSummary(filename, file_content, file_diff)));
        ...
        // Final Summary
        const [summarize_final_response, summarize_final_history] = await bot.chat(prompts.render_summarize(inputs), summarize_begin_history);
        if (summarize_final_response) {
            inputs.summary = summarize_final_response;
            const summarize_comment = `${summarize_final_response}\n\n---\n\nTips:\n- Reply to this comment to ask follow-up questions.\n- Type \`@gemini\` in a review comment to invite the bot to a conversation.`;
            await commenter.comment(summarize_comment, SUMMARIZE_TAG, 'replace');
        }
        // Final Release Notes
        const [release_notes_response] = await bot.chat(prompts.render_summarize_release_notes(inputs), summarize_final_history // Continue the conversation from the final summary
        );
        if (release_notes_response) {
            let message = '### Summary by Gemini\n\n' + release_notes_response;
            await commenter.update_description(context.payload.pull_request.number, message);
        }
        // Review Stage
        const [, review_begin_history] = await bot.chat(prompts.render_review_beginning(inputs), [] // Start a new conversation for the review stage
        );
        const reviewPromises = files_to_review.map(([filename, file_content, file_diff, patches]) => concurrency_limit(async () => {
            let file_review_history = review_begin_history;
            const ins = inputs.clone();
            ins.filename = filename;
            if (file_content) {
                ins.file_content = file_content;
                ...
                if (file_content_tokens < options.max_tokens_for_extra_content) {
                    try {
                        const [resp, new_history] = await bot.chat(prompts.render_review_file(ins), file_review_history);
                        file_review_history = new_history;
                        if (resp && !resp.includes('LGTM')) {
                            const tag = `<!-- gemini-review-file-${filename} -->`;
                            const comment = `${tag}\nReviewing existing code in: ${filename}\n\n${resp}`;
                            await commenter.comment(comment, tag, 'replace');
                        }
                    }
                    catch (error) {
                        core.warning(`review: error from gemini: ${error}`);
                    }
                }
            }
            if (file_diff) {
                ...
                if (file_diff_tokens < options.max_tokens_for_extra_content) {
                    try {
                        const [, new_history] = await bot.chat(prompts.render_review_file_diff(ins), file_review_history);
                        file_review_history = new_history;
                    }
                    catch (error) {
                        core.warning(`review: error from gemini: ${error}`);
            ...
            const [, patch_begin_history] = await bot.chat(prompts.render_review_patch_begin(ins), file_review_history);
            let patch_review_history = patch_begin_history;
            for (const [line, patch] of patches) {
                core.info(`Reviewing ${filename}:${line} with Gemini...`);
                ins.patch = patch;
                try {
                    ins.comment_chain =
                        (await commenter.get_conversation_chains_at_line(context.payload.pull_request.number, filename, line, COMMENT_REPLY_TAG)) || 'no previous comments';
                }
                catch (e) {
                    core.warning(`Failed to get comments: ${e}, skipping.`);
                }
                try {
                    const [response, new_history] = await bot.chat(prompts.render_review_patch(ins), patch_review_history);
...
const split_patch = (patch) => {
    if (!patch)
        return [];
    const pattern = /(^@@ -\d+,\d+ \+\d+,\d+ @@.*$)/gm;
    return patch.split(pattern).slice(1)
        .reduce((acc, val, i) => {
        if (i % 2 === 0)
            acc.push(val + (patch.split(pattern).slice(1)[i + 1] || ''));
        return acc;
    }, []);
};
const patch_comment_line = (patch) => {
    ...
    if (match) {
        ...
        const num_lines = parseInt(match[4], 10);
        return num_lines === 0 ? start - 1 : start + num_lines - 1;
    }
    return -1;
};
